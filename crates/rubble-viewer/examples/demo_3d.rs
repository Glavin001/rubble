...
fn scene_empty() -> Vec<RigidBodyDesc> {
    Vec::new()
}

fn scene_ground() -> Vec<RigidBodyDesc> {
    vec![
...
fn scene_dynamic_friction() -> Vec<RigidBodyDesc> {
...
fn scene_static_friction() -> Vec<RigidBodyDesc> {
...
fn scene_pyramid() -> Vec<RigidBodyDesc> {
    const SIZE: usize = 16;
...
fn scene_stack() -> Vec<RigidBodyDesc> {
    let mut descs = vec![box_desc(0.0, 0.0, 0.0, 100.0, 1.0, 100.0, 0.0, 0.5)];
    for i in 0..10 {
        descs.push(box_desc(
            0.0,
...
fn scene_stack_ratio() -> Vec<RigidBodyDesc> {
    let mut descs = vec![box_desc(0.0, 0.0, 0.0, 100.0, 1.0, 100.0, 0.0, 0.5)];
...
fn scene_scatter() -> Vec<RigidBodyDesc> {
...
fn scene_grid_10k_boxes() -> Vec<RigidBodyDesc> {
    let mut descs = vec![RigidBodyDesc {
        position: Vec3::ZERO,
        mass: 0.0,
        shape: ShapeDesc::Plane {
