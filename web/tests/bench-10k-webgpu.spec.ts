 ...
 *
 * Mirrors crates/rubble3d/examples/bench_10k.rs but runs in a real browser
 * with real WebGPU, measuring the actual WASM+WebGPU pipeline.
 *
 * Usage:
 *   cd web && npx playwright test tests/bench-10k-webgpu.spec.ts
 *
 * The test outputs JSON timing stats (matching the Rust bench format) to
 * stdout via console.log so you can capture it with:
...
test.describe("WebGPU 10k Grid Benchmark", () => {
  ...
  test("benchmark 10k grid scene", async ({ page }) => {
    ...
    // --- Warmup ---
    const warmupStartStep = await page.evaluate(
      () => window.__rubble_test?.stepCount ?? 0,
    );

    await page.waitForFunction(
      (target: number) => (window.__rubble_test?.stepCount ?? 0) >= target,
      warmupStartStep + WARMUP_STEPS,
      { timeout: 120_000 },
    );
    // eslint-disable-next-line no-console
    ...
    // --- Benchmark ---
    // Collect per-step timings by polling each step
    const timings: number[][] = [];

    const benchStartStep = await page.evaluate(
      () => window.__rubble_test?.stepCount ?? 0,
    );

    let lastSeenStep = benchStartStep;

    while (timings.length < BENCH_STEPS) {
      const sample = await page.evaluate(() => {
        const t = window.__rubble_test;
        return {
          stepCount: t?.stepCount ?? 0,
          timings: t?.lastStepTimingsMs ? Array.from(t.lastStepTimingsMs) : null,
          error: t?.error ?? null,
        };
      });

      if (sample.error) {
        throw new Error(`Simulation error at step ${sample.stepCount}: ${sample.error}`);
      }

      if (sample.stepCount > lastSeenStep && sample.timings) {
        timings.push(sample.timings);
        lastSeenStep = sample.stepCount;

        if (timings.length % 20 === 0) {
          const t = sample.timings;
          const stepTotal = t.reduce((a, b) => a + b, 0);
          // eslint-disable-next-line no-console
          console.log(
            `  step ${timings.length}: total=${stepTotal.toFixed(1)}ms ` +
